...
- [x] **Step 1: 写入只读/可恢复验收脚本**

脚本只接受显式的 `baseline`、`check`、`backup`、`rollback` 子命令；默认拒绝执行。脚本使用 `/etc/s-box/sb.json`、`/etc/s-box/sing-box` 和明确的 systemd unit 路径，若路径不存在只报告并返回非零，不猜测其他目录。
...
- [x] **Step 1: 复制当前配置并确认服务健康**

记录原配置哈希，先运行 `sing-box check` 和 `systemctl is-active`，任一失败则停止注入。

- [x] **Step 2: 注入临时无效配置并测试拒绝**

先把无效内容写入临时文件；在备份和健康检查均通过后，短暂替换正式配置以触发 systemd 的 `ExecStartPre` 守卫，随后立即恢复，确认拒绝命令非零且最终正式配置哈希不变。
